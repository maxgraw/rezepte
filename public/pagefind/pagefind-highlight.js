				...
				typeof Symbol === "function" && typeof Symbol.iterator === "symbol"
					...
					: (obj) =>
							...
							obj !== Symbol.prototype
								? "symbol"
								: typeof obj;
			var classCallCheck = (instance, Constructor) => {
					...
					[
						{
							...
							value: function getContexts() {
								...
								ctx.forEach((ctx2) => {
									var isDescendant =
										filteredCtx.filter((contexts) => contexts.contains(ctx2))
											.length > 0;
								...
								while (retrieveNodes()) {
									if (this.iframes) {
										this.forEachIframe(
											ctx,
											(currIfr) =>
												_this5.checkIframeFilter(node, prevNode, currIfr, ifr),
											(con) => {
												_this5
					...
					},
					{
						...
						value: function checkRanges(array) {
							...
							array
								...
								.forEach((item) => {
									var _callNoMatchOnInvalid = this.callNoMatchOnInvalidRanges(
											item,
											last,
										),
						...
						value: function wrapRangeFromIndex(
							...
							this.getTextNodes((dict) => {
								...
								ranges.forEach((range, counter) => {
									var _checkWhitespaceRange = this.checkWhitespaceRanges(
											range,
											originalLength,
											dict.value,
										),
							...
							var _getSeparatedKeywords = this.getSeparatedKeywords(
								...
								handler = function handler2(kw) {
									...
									_this10[fn](
										...
										1,
										(term, node) =>
											_this10.opt.filter(node, kw, totalMatches, matches),
										(element) => {
											matches++;
											totalMatches++;
